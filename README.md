...
- Week 2: 사례 시스템 실행 완료·주차 과제 `2/6` - 실제 상가·빌딩 1건으로 비공개 앱, 프로젝트 Skill, 공적 근거·임대차 대조, 3단계 승인, 비공개 DB 저장과 공개 비식별 요약 연동까지 검증
- Week 2 남은 과제: 챌린지 인증, 사례글 게시, 발표·피어 피드백, 다음 실험 확정. 실제 Drive 마스킹 폴더와 계약서 PDF 직접 입력, 인쇄/PDF 분리, 두 번째 매물 재실행은 후속 검증
...
- Vercel 계정에서 기존 토큰 폐기는 대표님 결정으로 보류
- 비공개 앱은 별도 Vercel 프로젝트의 보호된 Preview로 배포하고 Vercel Authentication 접근 차단을 확인
- Node.js: `24.14.0`
- Codex CLI: `0.145.0`
